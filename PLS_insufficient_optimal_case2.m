...
A_range=[angle(x1-x_min+1j*(y1-y_min))  angle(x1-x0+1j*(y1-y0))];
% A_range=[-1 0.7];
Q_range=[0 1000];
A=mean(A_range);
Q=mean(Q_range);
...
%                 disp([bound2(1),bound2(2)]);
        ...
        if dis_x0>tra_ds 
            ...
            % 最佳拉力范围的选择  Q↑ r(s)↓
            if angle(x-x1+1j*(y-y1))<angle(x_min-x1+1j*(y_min-y1)) & (y<0)
...
%                 disp('---------circle--------------');
            ...
            ang1 = pi-angle(x1-x_min+1j*(y1-y_min));
            ang0 = pi-angle(x1-x0+1j*(y1-y0));
            ang_bac = ang1-ang0;
            ang_alpha = ang0+angle(x-x_min+1j*(y-y_min));
            r_min0 = norm([x1-x_min, y1-y_min]);
...
%                     disp('increasing Q');
                    ...
                    Q=mean(Q_range);
                    break;
            elseif  angle(x-x_min+1j*(y-y_min))<=angle(x0-x_min+1j*(y0-y_min)) %last_dis_x1<=dis_x1  %angle(x-xe+1j*(y-ye))>angle(x1-xe+1j*(y1-ye))  
                    % 需减小Q 所以Q在下1/2处
%                     disp('...........................................');
%                     disp('decreasing Q');
                    Q_range(2)=Q;
...
while 1
    ...
    R_opt=R_opt+PLS_real_R(r,re,H,P_lambda(ii+1))*ds/V;
    if s<=s_A
    ...
    if sqrt((x-x0)^2+(y-y0)^2)<=tra_ds
        break;
    end
end

% figure;
% plot([x1 x1],[y1 y1],'^');hold on;
